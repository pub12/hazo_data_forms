...
export default function NestedSectionsPage() {
  ...
  const [collapsed, set_collapsed] = useState<string[]>([]);
  ...
  const toggle_collapse = (section: string) => {
    set_collapsed((prev) =>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Nested Sections Demo</h1>
        <p className="text-muted-foreground mt-2">
          Demonstrates complex nested sections like a tax return form with
      ...
      <div className="flex gap-2 mb-4">
        <button
          onClick={() => toggle_collapse("Personal Information")}
          className="px-3 py-1 text-sm border rounded-md hover:bg-muted"
        >
          Toggle Personal Info
        </button>
        <button
          onClick={() => toggle_collapse("Addresses")}
          className="px-3 py-1 text-sm border rounded-md hover:bg-muted"
        >
          Toggle Addresses
        </button>
        <button
          onClick={() => toggle_collapse("Contact & Banking")}
          className="px-3 py-1 text-sm border rounded-md hover:bg-muted"
        >
          Toggle Contact & Banking
        </button>
      </div>
