...
### Fixed
- **File Upload Icons in Stacked Layout**: Fixed file upload/attachment icons (FileManagerButton) not rendering for fields using stacked layout (default). Previously, file icons only appeared for fields with `label_position: "inline"`. Now all 7 field types that support file attachments (text, number, currency, date, option, boolean, percentage) correctly display the file manager button in both inline and stacked layouts.
